...
#'
#'
holidays_heat_map <- function(data, normalize=FALSE, holidaytype=c("Sportferien", "Fruehlingsferien", "Sommerferien", "Herbstferien", "Weihnachtsferien")) {
  ...
  if (a || b || c) {
    ...
    return(message("Own or customised datasets are not compatible with this function. Choose a dataset from this package. E.g. holidays_2020_long, holidays_2020_long_major_regions, or holidays_2020_long_switzerland. For more details, check the function documentation at ?holidays_heat_map."))
  ...
  df.temp_a <- df.3[df.3$Ferientyp %in% holiday,]
  df.temp_b <- df.3[!(df.3$Ferientyp %in% holiday),]
  ...
  #### Step 4 - Plot data ####
  df.plot <- df.4
  n <- length(unique(df.plot$Primary.key))
